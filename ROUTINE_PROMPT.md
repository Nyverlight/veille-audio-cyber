...
## 1. Préparation
- Exécute `bash setup.sh` (installe les dépendances et edge-tts).
...
- **Fusionne** les actualités qui parlent du même sujet, même venant de
  sources différentes (compare le fond, pas seulement le titre). Une même
  attaque, fuite ou vulnérabilité est souvent reprise par plusieurs médias.
- **Hiérarchise** selon l'importance pour un professionnel de la sécurité :
  1. Vulnérabilités **activement exploitées** (exploitation "in the wild",
     ajout au catalogue CISA KEV, PoC public sur des produits répandus).
  2. Incidents majeurs : compromissions, fuites de données, ransomware visant
     des organisations notables, campagnes d'acteurs étatiques.
  3. Tendances de fond, analyses de threat intelligence, réglementation
     (NIS2, DORA, etc.).
- **Regroupe les bulletins de vulnérabilités** (CERT-FR, CISA, ICS) : ne lis
  PAS chaque CVE une par une. Fais-en un segment synthétique du type
  « côté vulnérabilités à corriger en priorité cette semaine… » en ne citant
  que les plus critiques ou les plus pertinentes (produits très déployés,
  équipements industriels / OT, sécurité physique).
- **Écarte** le promotionnel, l'anecdotique, les articles "sponsorisés" et le
  contenu sans intérêt pour rester à jour.
...
- Pour chaque sujet retenu, rédige un résumé court, clair et factuel
  ...
  (ex. « selon Bleeping Computer », « d'après le CERT-FR »).
...
- Les noms propres anglais, les noms de produits/éditeurs et les identifiants
  de vulnérabilités (ex. CVE-2026-12345) restent **tels quels**. Pour une
  référence CVE, tu peux la lire « C-V-E » suivi des chiffres.
...
- Exécute :
  `python scripts/make_audio.py --config config/config.json --in work/script.txt --out audio/cyber-DATE.mp3`
  (remplace DATE par la date réelle).

## 6. Mise à jour du flux podcast
- Lis le contenu de `work/desc.txt` (variable DESC).
- Exécute :
  `python scripts/update_feed.py --config config/config.json --audio audio/cyber-DATE.mp3 --title "Veille Cyber - DATE" --desc "DESC"`

## 7. Publication
- Ajoute au suivi git les fichiers modifiés : `feed.xml` et le dossier
  `audio/` (y compris les suppressions dues à la rétention).
- Commite avec un message du type « Veille Cyber - DATE » et pousse sur la
  branche `main`.
